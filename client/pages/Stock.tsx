        ...
        <TabsContent value="history" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <p className="text-sm text-gray-600">
                Toutes les entrées et sorties de stock pour cette ferme
              </p>
            </CardHeader>
            <CardContent>
                ...
                <div className="space-y-2">
                  ...
                  <div className="bg-green-50 rounded-lg p-4">
                    <Table>
                      ...
                      <TableBody>
                        {filteredStocks
                          .filter(stock => stock.secteurId === user?.fermeId || (!user?.fermeId && true))
                          .sort((a, b) => new Date(b.lastUpdated).getTime() - new Date(a.lastUpdated).getTime())
                          .map((stock) => (
                          <TableRow key={`entry-${stock.id}`}>
                            <TableCell>{formatDate(stock.lastUpdated)}</TableCell>
                            <TableCell className="font-medium">{stock.item}</TableCell>
                            <TableCell className="text-green-600 font-semibold">+{stock.quantity}</TableCell>
                            <TableCell>
                              <Badge className="bg-green-100 text-green-800">
                                {stock.quantity > 0 ? 'Création article' : 'Ajout stock'}
                              </Badge>
                            </TableCell>
                            <TableCell className="text-sm text-gray-600">
                        ...
                        {filteredStocks.filter(stock => stock.secteurId === user?.fermeId || (!user?.fermeId && true)).length === 0 && (
                          <TableRow>
                  ...
                  <div className="bg-orange-50 rounded-lg p-4">
                    <Table>
                      ...
                      <TableBody>
                        {workers
                          .filter(worker =>
                            worker.fermeId === user?.fermeId || (!user?.fermeId && true)
                          )
                          .flatMap(worker =>
                          ...
                          <TableRow key={`allocation-${allocation.id || index}`}>
                            <TableCell>{formatDate(allocation.allocatedAt)}</TableCell>
                            <TableCell className="font-medium">{allocation.itemName}</TableCell>
                            <TableCell>{allocation.workerName}</TableCell>
                            <TableCell className="text-orange-600 font-semibold">-1</TableCell>
                            <TableCell>
                              <Badge
                                className={`${
                              ...
                              >
                                {allocation.status === 'allocated' ? 'Alloué' : 'Retourné'}
                              </Badge>
                            </TableCell>
                          </TableRow>
                        ))}
                        {workers
                          .filter(worker => worker.fermeId === user?.fermeId || (!user?.fermeId && true))
                          .flatMap(worker => worker.allocatedItems || []).length === 0 && (
                          <TableRow>
                            <TableCell colSpan={5} className="text-center text-gray-500 py-4">
