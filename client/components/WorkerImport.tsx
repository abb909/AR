...
export const WorkerImport: React.FC<WorkerImportProps> = ({
  ...
  const validateWorkerData = (data: any, rowIndex: number): ImportedWorker => {
    ...
    // Process EPONGE and preserve raw value - ALWAYS from column 10
    console.log(`🚀 Starting EPONGE processing for row ${rowIndex}:`, {
      dataEponge: data.eponge,
      epongeExists: 'eponge' in data,
      dataKeys: Object.keys(data),
      epongeType: typeof data.eponge,
      forceProcessing: true,
      sourceColumn: 'Column 10 (forced)'
    });

    // Force process eponge from column 10, regardless of other validations
    if (data.eponge !== undefined && data.eponge !== null) {
      const rawValue = String(data.eponge).trim();
    ...
    // FINAL SAFEGUARD: Ensure eponge is always processed from column 10
    console.log(`🛡️ EPONGE Final safeguard check for row ${rowIndex}:`, {
      currentEponge: worker.eponge,
      hasEpongeSet: !!worker.eponge,
      forceReprocessing: !worker.eponge
    });

    if (!worker.eponge || worker.eponge === '-') {
      console.log(`🔄 Reprocessing eponge from column 10 for row ${rowIndex}`);
      const rawColumn10 = data.eponge;
      if (rawColumn10 !== undefined && rawColumn10 !== null && String(rawColumn10).trim() !== '') {
        const rawValue = String(rawColumn10).trim();
        const epongeValue = rawValue.toLowerCase().trim();
        const ouiVariations = ['oui', 'yes', 'y', 'o', '1', 'true', 'vrai', 'si'];
        const nonVariations = ['non', 'no', 'n', '0', 'false', 'faux'];
        ...
        if (ouiVariations.includes(epongeValue)) {
          worker.eponge = 'oui';
          console.log(`✅ SAFEGUARD: EPONGE set to "oui" for ${worker.nom || 'worker'}`);
        } else if (nonVariations.includes(epongeValue)) {
          worker.eponge = 'non';
          console.log(`✅ SAFEGUARD: EPONGE set to "non" for ${worker.nom || 'worker'}`);
        } else {
          worker.eponge = rawValue;
          console.log(`⚠️ SAFEGUARD: EPONGE set to raw value "${rawValue}" for ${worker.nom || 'worker'}`);
        }
      } else {
        worker.eponge = '-';
        console.log(`➖ SAFEGUARD: EPONGE set to "-" (empty) for ${worker.nom || 'worker'}`);
      }
    }
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const normalizedData = jsonData.map((row: any, index: number) => {
        ...
        // Get column 10 (index 9) value for eponge - direct access by position
        const rowValues = Object.values(row);
        const column10Value = rowValues[9]; // Column 10 is index 9 (0-based)
        console.log(`🧽 Row ${index + 1} Column 10 (eponge) direct access:`, {
          value: column10Value,
          type: typeof column10Value,
          isEmpty: !column10Value,
          isUndefined: column10Value === undefined,
          isNull: column10Value === null,
        ...
        // Force set eponge from column 10
        normalizedRow.eponge = column10Value;
        console.log(`🎯 Forced eponge from column 10 for row ${index + 1}:`, column10Value);
        ...
        <Alert className="bg-yellow-50 border-yellow-200">
          ...
          <AlertDescription>
            ...
            <div className="mt-2 text-xs text-yellow-600">
              ℹ️ Ces problèmes ont ��té automatiquement corrigés. Les ouvriers seront importés sans assignation de chambre.
            </div>
          </AlertDescription>
        </Alert>
      )}
